...
| Q87 | 2026-09-23 | The renderer classifies each `files[]` entry as `code`, `docs`, or `other`, groups the per-file diffs by kind, and derives `docs-only` when no file is `code`; `files[].kind` and `change_classification` remain as overrides | The Q6 rule lived as prose in three skills for the agent to apply; in the renderer it is applied once and tested. A third kind keeps lockfiles, images, and dotfiles out of the Docs group without changing which changes count as docs-only |
