...
VS Code seeding merges keys into the user `settings.json` (preserving all others): `chat.useClaudeMdFile: true`, the PRD agent symlink into the profile `User/prompts/` dir (fallback: `chat.agentFilesLocations`), and the localml provider under `github.copilot.chat.customOAIModels` (baseUrl `http://127.0.0.1:8080/v1`, model id placeholder). The API key lives in VS Code secret storage and cannot be file-seeded — the runbook covers that prompt (any non-empty string; localml ignores it).
