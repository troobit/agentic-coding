...
- Interactive-entry only: no router (nextup, engage, orbit) dispatches the skill autonomously; headless invocation is out of contract. The cloud surface only ever gets the report-only agent.
...
- Guardrail edits citing SJ rule IDs live in engage, starwave-smolspec, starwave-tasks, fix-bug, next-task and make-it-so. nextup is deliberately excluded — it is a pure router that performs no spec edits.
